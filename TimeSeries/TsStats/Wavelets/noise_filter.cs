...
namespace HC.Analytics.TimeSeries.TsStats.Wavelets
{
    ...
    public class noise_filter
    {
        ...
        /**
          ...
          index field.
         */
        private class sort_by_index : generic_sort
        {
        ...
        /**
          ...
          val filed.
         */
        private class sort_by_val : generic_sort
        {
        ...
        {
            const int min_size = 64;  // minimum spectrum size
         ...
         */
        public void filter_time_series(
            double[] ts0,
            out double[] results,
            out double[] noise)
        {
            results = (double[])ts0.Clone();
            noise = new double[results.Length];
            var haar = new inplace_haar();
            haar.wavelet_calc(results);
            haar.order();
            gaussian_filter(results, noise);
            haar.inverse();
