 ...
 * PrintToScreen.WriteLine("The instance: " + inst); <br>
 *
 * ... <br>
 * </code><p>
 *
...
namespace HC.Analytics.MachineLearning.NaiveBayes.Impl2_Weka
{
    ...
    public class Instance
    {
        ...
        protected static double MISSING_VALUE = double.NaN;
        ...
        public /*@pure@*/ bool classIsMissing()
        {
        ...
        //@ also ensures \result is Instance;
         ...
         */
        //@ requires m_Dataset != null;
        public /*@pure@*/ FastVector.FastVectorEnumeration enumerateAttributes()
        {
         ...
         */
        //@ requires m_Dataset != null;
        public /*@pure@*/ bool equalHeaders(Instance inst)
        {
         ...
         */
        //@ requires m_Dataset != null;
        public /*@pure@*/ bool hasMissingValue()
        {
         ...
         */
        public /*@pure@*/ bool isMissing(int attIndex)
        {
         ...
         * @param indexOfIndex the index of the attribute's index 
         */
        public /*@pure@*/ bool isMissingSparse(int indexOfIndex)
        {
         ...
         */
        public /*@pure@*/ bool isMissing(Attribute att)
        {
         ...
         */
        public static /*@pure@*/ bool isMissingValue(double val)
        {
         ...
         * @exception Exception if the attribute is not
         ...
         * the value is added to the attribute).
         * @exception Exception if the dataset is not set
         * @exception Exception if the selected
         ...
         * is a string attribute and the value can't be found,
         * the value is added to the attribute).
         * @exception Exception if the the attribute is not
         * nominal or a string, or the value couldn't be found for a nominal
         * attribute 
         */
         ...
         */
        //@ requires m_Dataset != null;
        public  /*@pure@*/ string stringValue(int attIndex)
        {
         ...
         * to a dataset.
         */
        public  /*@pure@*/ string stringValue(Attribute att)
        {
            ...
            switch (att.type())
            {
                ...
                default:
                    throw new Exception("Attribute isn't nominal, string or date!");
            }
        }
        ...
        public double[] todoubleArray()
        {
            ...
            Array.Copy(m_AttValues, 0, newValues, 0,
                     m_AttValues.Length);
            return newValues;
        }
         ...
         */
        public  /*@pure@*/ string ToString(int attIndex)
        {
            ...
            else
            {
                if (m_Dataset == null)
                {
                    text.Append(m_AttValues[attIndex]);
                }
                else
                {
        ...
        void forceDeleteAttributeAt(int position)
        {
            ...
            {
                Array.Copy(m_AttValues, position + 1,
                       newValues, position,
        ...
        void forceInsertAttributeAt(int position)
        {
            ...
            Array.Copy(m_AttValues, position, newValues,
        ...
        public static void main(string[] options)
        {
            ...
            try
            {
                ...
                // Print the instance
                PrintToScreen.WriteLine("The instance: " + inst);

                // Print the first attribute
                PrintToScreen.WriteLine("First attribute: " + inst.attribute(0));

                // Print the class attribute
                PrintToScreen.WriteLine("Class attribute: " + inst.classAttribute());

                // Print the class index
                PrintToScreen.WriteLine("Class index: " + inst.classIndex());

                // Say if class is missing
                PrintToScreen.WriteLine("Class is missing: " + inst.classIsMissing());

                // Print the instance's class value in internal format
                PrintToScreen.WriteLine("Class value (internal format): " + inst.classValue());

                // Print a shallow copy of this instance
                Instance copy = (Instance)inst.copy();
                PrintToScreen.WriteLine("Shallow copy: " + copy);

                // Set dataset for shallow copy
                copy.setDataset(inst.dataset());
                PrintToScreen.WriteLine("Shallow copy with dataset set: " + copy);
                ...
                copy.setDataset(inst.dataset());
                PrintToScreen.WriteLine("Copy with first attribute deleted and inserted: " + copy);

                // Enumerate attributes (leaving out the class attribute)
                PrintToScreen.WriteLine("Enumerating attributes (leaving out class):");
                FastVector.FastVectorEnumeration enu = inst.enumerateAttributes();
                while (enu.hasMoreElements())
                {
                    Attribute att = (Attribute)enu.nextElement();
                    PrintToScreen.WriteLine(att);
                }

                // Headers are equivalent?
                PrintToScreen.WriteLine("Header of original and copy equivalent: " +
                       inst.equalHeaders(copy));

                // UnitTest for missing values
                PrintToScreen.WriteLine("Length of copy missing: " + copy.isMissing(Length));
                PrintToScreen.WriteLine("Weight of copy missing: " + copy.isMissing(weight.index()));
                PrintToScreen.WriteLine("Length of copy missing: " +
                       Instance.isMissingValue(copy.value(Length)));
                PrintToScreen.WriteLine("Missing value coded as: " + Instance.missingValue());

                // Prints number of attributes and classes
                PrintToScreen.WriteLine("Number of attributes: " + copy.numAttributes());
                PrintToScreen.WriteLine("Number of classes: " + copy.numClasses());

                // Replace missing values
                double[] meansAndModes = { 2, 3, 0 };
                copy.replaceMissingValues(meansAndModes);
                PrintToScreen.WriteLine("Copy with missing value replaced: " + copy);

                // Setting and getting values and weights
                copy.setClassMissing();
                PrintToScreen.WriteLine("Copy with missing class: " + copy);
                copy.setClassValue(0);
                PrintToScreen.WriteLine("Copy with class value set to first value: " + copy);
                copy.setClassValue("third");
                PrintToScreen.WriteLine("Copy with class value set to \"third\": " + copy);
                copy.setMissing(1);
                PrintToScreen.WriteLine("Copy with second attribute set to be missing: " + copy);
                copy.setMissing(Length);
                PrintToScreen.WriteLine("Copy with Length set to be missing: " + copy);
                copy.setValue(0, 0);
                PrintToScreen.WriteLine("Copy with first attribute set to 0: " + copy);
                copy.setValue(weight, 1);
                PrintToScreen.WriteLine("Copy with weight attribute set to 1: " + copy);
                copy.setValue(position, "second");
                PrintToScreen.WriteLine("Copy with position set to \"second\": " + copy);
                copy.setValue(2, "first");
                PrintToScreen.WriteLine("Copy with last attribute set to \"first\": " + copy);
                PrintToScreen.WriteLine("Current weight of instance copy: " + copy.weight());
                copy.setWeight(2);
                PrintToScreen.WriteLine("Current weight of instance copy (set to 2): " + copy.weight());
                PrintToScreen.WriteLine("Last value of copy: " + copy.ToString(2));
                PrintToScreen.WriteLine("Value of position for copy: " + copy.ToString(position));
                PrintToScreen.WriteLine("Last value of copy (internal format): " + copy.value(2));
                PrintToScreen.WriteLine("Value of position for copy (internal format): " +
                       copy.value(position));
            }
            catch (Exception e)
            {
