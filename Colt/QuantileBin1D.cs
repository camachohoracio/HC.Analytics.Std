...
namespace HC.Analytics.Colt
{
    ...
    //////import RngWrapper;
    //////import quantile.DoubleQuantileFinder;
    //////import quantile.QuantileFinderFactory;
    /**
    ...
    <li><i>quantiles</i> - the number of quantiles to be computed; in <tt>[0,int.MaxValue]</tt>.

    <li><i>is N known?</i> - specifies whether the exact size of the dataset over which quantiles are to be computed is known.

    <li>N<sub>Max</sub> - the exact dataset size, if known. Otherwise, an upper limit on the dataset size. If no upper limit is known set to infinity (<tt>long.MaxValue</tt>).
    </ul>
    ...
    */
    ...
    {
        public DoubleQuantileFinder finder;
        /**
        ...
        /**
         * Equivalent to <tt>new QuantileBin1D(false, long.MaxValue, epsilon, 0.001, 10000, new DRand(new DateTime())</tt>.
         */

        public QuantileBin1D(double epsilon)
            : this(false, long.MaxValue, epsilon, 0.001, 10000, new RngWrapper())
        {
        }
        ...
        public QuantileBin1D(bool known_N, long N, double epsilon, double delta, int quantiles,
         ...
         * 			If such an upper limit is a-priori unknown, then set <tt>N = long.MaxValue</tt>.
         * <p>
        ...
        public QuantileBin1D(bool known_N, long N, double epsilon, double delta, int quantiles,
                             RngWrapper randomGenerator, bool hasSumOfLogarithms, bool hasSumOfInversions,
                             int maxOrderForSumOfPowers)
            : base(hasSumOfLogarithms, hasSumOfInversions, maxOrderForSumOfPowers)
        {
            finder = QuantileFinderFactory.newDoubleQuantileFinder(
                known_N,
                N,
                epsilon,
                delta,
        ...
        public new void addAllOfFromTo(DoubleArrayList list, int from, int to)
        {
            ...
            if (finder != null)
            {
                finder.addAllOfFromTo(list, from, to);
            }
        }
        ...
        {
            return quantiles(new DoubleArrayList(new[] {phi})).get(0);
        }
        ...
        */
        ...
        public MightyStaticBin1D[] splitApproximately(DoubleArrayList percentages, int k)
        {
            ...
            double[] quantilesArr = quantiles(new DoubleArrayList(subBins)).elements();
            ...
            for (int i = 0; i < noOfBins; i++)
            {
                ...
                double binMax2 = binMin;
                binMin = safe_min;

                // fill statistics
                splitBins[i] = new MightyStaticBin1D(m_hasSumOfLogarithms, m_hasSumOfInversions, maxOrderForSumOfPowers);
                if (binSize > 0)
                {
                    splitBins[i].m_size = binSize;
                    splitBins[i].m_min = binMin;
                    splitBins[i].m_max = binMax2;
                    splitBins[i].m_sum = binSum;
                    splitBins[i].m_sum_xx = binSumOfSquares;
                    splitBins[i].m_sumOfLogarithms = binSumOfLogarithms;
                    splitBins[i].m_sumOfInversions = binSumOfInversions;
                    splitBins[i].m_sumOfPowers = binSumOfPowers;
                }
                /*
                double binMean = binSum / binSize;
                PrintToScreen.WriteLine("size="+binSize);
                PrintToScreen.WriteLine("Min="+binMin);
                PrintToScreen.WriteLine("Max="+binMax);
                PrintToScreen.WriteLine("mean="+binMean);
                PrintToScreen.WriteLine("sum_x="+binSum);
                PrintToScreen.WriteLine("sum_xx="+binSumOfSquares);
                PrintToScreen.WriteLine("rms="+Math.Sqrt(binSumOfSquares / binSize));
        ...
        */
        ...
        public MightyStaticBin1D[] splitApproximately(IAxis axis, int k)
        {
