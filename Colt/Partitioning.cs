...
namespace HC.Analytics.Colt
{
    ...
    */
     ...
     *
     * @see Partitioning "Partitioning arrays (provides more documentation)"
     *
    ...
    [Serializable]
    public class Partitioning : Object
    {
        /**
         * Makes this class non instantiable, but still let's others inherit from it.
         */
        /**
        Same as {@link Partitioning#partition(int[],int,int,int[],int,int,int[])}
        ...
        */
            ...
            int[] splitIndexes)
        {
            ...
            int[] g = rowIndexes;
            Swapper swapper = new Swapper5_(g);

            // Compare splitter[a] with columnView[rowIndexes[b]]
            ObjectMatrix1D columnView = matrix.viewColumn(column);
            IntComparator comp = new IntComparator4_(splitters,
                                                     columnView,
                                                     g);

            // Compare columnView[rowIndexes[a]] with columnView[rowIndexes[b]]
            IntComparator comp2 = new IntComparator5_(
                splitters,
                columnView,
                g);

            // Compare splitter[a] with splitter[b]
            IntComparator comp3 = new IntComparator6_(
                splitters,
                columnView,
                g);

            // generic partitioning does the main work of reordering row indexes
            Partitioning2.genericPartition(rowFrom, rowTo, splitFrom, splitTo, splitIndexes, comp, comp2, comp3, swapper);
        }
        ...
        */
                                          ...
                                          Object[] splitters, int splitFrom, int splitTo, int[] splitIndexes)
        {
            ...
            if (splitFrom==splitTo) { // we don't really have a choice
                ...
                medianIndex = Sorting.binarySearchFromTo(splitters,column.getQuick(m),splitFrom,splitTo);
            ...
            // Optimisation: Handle special cases to cut down recursions.
