...
namespace HC.Analytics.Colt
{
    ...
    */
    ...
    <p>
    A matrix has a number of rows_ and columns_, which are assigned upon instance construction - The matrix's size is then <tt>Rows()*Columns()</tt>.
    ...
    public abstract class ObjectMatrix2D : AbstractMatrix2D
    {
        ...
        */
            ...
            ObjectFunction f)
        {
            ...
            Object a = f.Apply(getQuick(m_intRows - 1, m_intColumns - 1));
        ...
        */
         ...
         * and have exactly the same number of rows_ and columns_ as the receiver.
         * <p>
        ...
        public ObjectMatrix2D assign(Object[,] values)
        {
            ...
            {
                throw new ArgumentException("Must have same number of rows_: rows_=" + values.GetLength(0) + "Rows()=" +
                                            Rows());
            }
            for (int row = m_intRows; --row >= 0;)
            {
                Object[] currentRow = ArrayHelper.GetRowCopy(values, row);
                if (currentRow.Length != m_intColumns)
                {
                    throw new ArgumentException("Must have same number of columns_ in every row: columns_=" +
        ...
        */
         ...
         * Both matrices must have the same number of rows_ and columns_.
        ...
        */
        ...
        public bool Equals(Object otherObj, bool testForEquality)
        {
            ...
            else
            {
                for (int row = m_intRows; --row >= 0;)
                {
                    ...
                    {
                        if (
                            !(getQuick(row, column) == null
                                  ? other.getQuick(row, column) == null
                                  : getQuick(row, column).Equals(other.getQuick(row, column))))
                        {
                            return false;
                        }
                    }
        ...
        This implementation fills like <tt>for (row = 0..rows_-1) for (column = 0..columns_-1) do ... </tt>.
        ...
        */
        ...
        public bool haveSharedCells(ObjectMatrix2D other)
        {
            if (other == null)
            {
                return false;
        ...
        public bool haveSharedCellsRaw(ObjectMatrix2D other)
        {
            return false;
        }

        /**
         * Construct and returns a new empty matrix <i>of the same dynamic type</i> as the receiver, having the same number of rows_ and columns_.
        ...
        /**
         * Construct and returns a new empty matrix <i>of the same dynamic type</i> as the receiver, having the specified number of rows_ and columns_.
         * For example, if the receiver is an instance of type <tt>DenseObjectMatrix2D</tt> the new matrix must also be of type <tt>DenseObjectMatrix2D</tt>,
         * if the receiver is an instance of type <tt>SparseObjectMatrix2D</tt> the new matrix must also be of type <tt>SparseObjectMatrix2D</tt>, etc.
         * In general, the new matrix should have internal parametrization as similar as possible.
         *
         * @param rows_ the number of rows_ the matrix shall have.
         * @param columns_ the number of columns_ the matrix shall have.
         * @return  a new empty matrix of the same dynamic type.
         */
        public abstract ObjectMatrix2D like(int rows_, int columns_);
        /**
         ...
         * and has the same number of rows_ and columns_ as the receiver.
         * <p>
         * The values are copied. So subsequent changes in <tt>values</tt> are not reflected in the matrix, and vice-versa.
         *
        ...
        {
            Object[,] values = new Object[m_intRows,m_intColumns];
            for (int row = m_intRows; --row >= 0;)
            {
                for (int column = m_intColumns; --column >= 0;)
                {
        ...
        /**
        Constructs and returns a new <i>slice view</i> representing the rows_ of the given column.
        ...
        */
        ...
        The view has both dimensions exchanged; what used to be columns_ become rows_, what used to be rows_ become columns_.
        ...
        */
        ...
        /**
        Constructs and returns a new <i>slice view</i> representing the columns_ of the given row.
        The returned view is backed by this matrix, so changes in the returned view are reflected in this matrix, and vice-versa.
        To obtain a slice view on subranges, construct a sub-ranging view (<tt>viewPart(...)</tt>), then apply this method to the sub-range view.
        <p> 
        <b>Example:</b> 
        ...
        */
        ...
        The returned view is backed by this matrix, so changes in the returned view are reflected in this matrix, and vice-versa. 
        <p>
        To indicate "all" rows_ or "all columns_", simply set the respective parameter
        @param  rowIndexes   The rows_ of the cells that shall be visible in the new view. To indicate that <i>all</i> rows_ shall be visible, simply set this parameter to <tt>null</tt>.
        @param  columnIndexes   The columns_ of the cells that shall be visible in the new view. To indicate that <i>all</i> columns_ shall be visible, simply set this parameter to <tt>null</tt>.
        @return the new view.
        ...
        */
        ...
        /**
        Constructs and returns a new <i>selection view</i> that is a matrix holding all <b>rows_</b> matching the given condition.
        Applies the condition to each row and takes only those row where <tt>condition.Apply(viewRow(i))</tt> yields <tt>true</tt>.
        To match columns_, use a dice view.
        <p>
        <b>Example:</b>
        <br>
        <pre>
        // extract and view all rows_ which have a value < threshold in the first column (representing "age")
         Object threshold = 16;
        matrix.viewSelection( 
        &nbsp;&nbsp;&nbsp;new ObjectMatrix1DProcedure() {
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;public  bool Apply(ObjectMatrix1D m) { return m.get(0) < threshold; }
        &nbsp;&nbsp;&nbsp;}
        );

        // extract and view all rows_ with RMS < threshold
        // The RMS (Root-Mean-Square) is a measure of the average "size" of the elements of a data sequence.
        matrix = 0 1 2 3
         Object threshold = 0.5;
        matrix.viewSelection( 
        &nbsp;&nbsp;&nbsp;new ObjectMatrix1DProcedure() {
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;public  bool Apply(ObjectMatrix1D m) { return Math.Sqrt(m.aggregate(Functions.plus,Functions.square) / m.Size()) < threshold; }
        &nbsp;&nbsp;&nbsp;}
        );
        </pre>
        For further examples, see the <a href="package-summary.html#FunctionObjects">//package doc</a>.
        ...
        */
        ...
        public ObjectMatrix2D viewSelection(ObjectMatrix1DProcedure condition)
        {
            ...
            matches.trimToSize();
            return viewSelection(matches.elements(), null); // take all columns_
        }
        ...
        /**
        Sorts the matrix rows_ into ascending order, according to the <i>natural ordering</i> of the matrix values in the given column.
        This sort is guaranteed to be <i>stable</i>.
        For further information, see {@link Appendobjectalgo.Sorting#Sort(ObjectMatrix2D,int)}.
        ...
        */
        ...
        {
            return SortingObjectAlgo.mergeSort.Sort(this, column);
        }

        /**
        Constructs and returns a new <i>stride view</i> which is a sub matrix consisting of every i-th cell.
        More specifically, the view has <tt>Rows()/rowStride</tt> rows_ and <tt>Columns()/columnStride</tt> columns_ holding cells <tt>get(i*rowStride,j*columnStride)</tt> for all <tt>i = 0..Rows()/rowStride - 1, j = 0..Columns()/columnStride - 1</tt>.
        The returned view is backed by this matrix, so changes in the returned view are reflected in this matrix, and vice-versa.
        ...
        */
         ...
         * <pre>
         * for (int row=rows_; --row >=0;) {
         *    for (int column=columns_; --column >= 0;) {
