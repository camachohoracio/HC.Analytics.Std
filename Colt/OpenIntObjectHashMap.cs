...
namespace HC.Analytics.Colt
{
    ...
    */
    ...
    [Serializable]
    public class OpenIntObjectHashMap : AbstractIntObjectMap
    {
        /**
        * The hash table keys.
        * @serial
        */
        ...
        {
            new ByteArrayList(state).fillFromToWith(0, state.Length - 1, FREE);

            new ObjectArrayList(m_values).fillFromToWith(0, state.Length - 1, null); // delta
        ...
        {
            OpenIntObjectHashMap copy = (OpenIntObjectHashMap) base.Clone();
        ...
        public new bool forEachPair(IntObjectProcedure procedure)
        {
            for (int i = table.Length; i-- > 0;)
            {
                if (state[i] == FULL)
        ...
        public new int keyOf(Object value)
        {
        ...
        */
        ...
        public new void pairsMatching(IntObjectProcedure condition, IntArrayList keyList, ObjectArrayList valueList)
        {
            keyList.Clear();
            valueList.Clear();
            ...
            for (int i = table.Length; i-- > 0;)
            {
                if (state[i] == FULL && condition.Apply(table[i], m_values[i]))
                {
                    keyList.Add(table[i]);
        ...
        public override bool put(int key, Object value)
        {
