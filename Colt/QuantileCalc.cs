...
namespace HC.Analytics.Colt
{
    ...
    */
    ...
    internal class QuantileCalc : Object
    {
        /**
         ...
         * This method is safer than using (long) Math.Ceiling(value), because of possible rounding error.
         */

        public static long ceiling(double value)
        {
            return (long) Math.Round(Math.Ceiling(value));
        }
        ...
        public static long[] known_N_compute_B_and_K_quick(long N, double epsilon)
        {
            ...
            for (int b = 2; b <= maxBuffers; b++)
            {
                ...
                while (h <= maxHeight && // skip heights until x<=0
                       (h - 2)*(Math.Round(binomial(b + h - 2, h - 1))) -
                       (Math.Round(binomial(b + h - 3, h - 3))) +
                       (Math.Round(binomial(b + h - 3, h - 2))) - c
                       > 0.0
                    )
                {
                    h++;
            ...
            else
            {
                ...
                intB = 1; // the only possible solution without violating the 
        ...
        public static long[] known_N_compute_B_and_K_slow(long N, double epsilon, double delta, int quantiles,
                                                          double[] returnSamplingRate)
        {
            ...
            for (long b = 2; b < maxBuffers; b++)
            {
                ...
                {
                    double dblBinomial = binomial(b + h - 2, h - 1);
                    long tmp = ceiling(N_double/dblBinomial);
                    if ((b*tmp < memory) &&
                        ((h - 2)*dblBinomial - binomial(b + h - 3, h - 3) + binomial(b + h - 3, h - 2)
                         <= c))
                    {
                        ret_k = tmp;
                        ret_b = b;
        ...
        public static void test_B_and_K_Calculation(string[] args)
        {
            ...
            {
                known_N = System.Convert.ToBoolean(args[0]);
            }
            ...
            PrintToScreen.WriteLine("mem [elements/1024]");
            PrintToScreen.WriteLine("***********************************");
            ...
            for (int q = 0; q < quantiles.Length; q++)
            {
                ...
                for (int s = 0; s < sizes.Length; s++)
                {
                    ...
                    for (int d = 0; d < deltas.Length; d++)
                    {
                        ...
                        PrintToScreen.WriteLine("      ------------------------------");
                        PrintToScreen.WriteLine("      computing for delta = " + delta);
                        for (int e = 0; e < epsilons.Length; e++)
                        {
                            ...
                            if (known_N)
                            {
                                Console.Write("," + returnSamplingRate[0]);
                            }
                            PrintToScreen.WriteLine(")");
                        }
                    }
        ...
        public static long[] unknown_N_compute_B_and_K(double epsilon, double delta, int quantiles)
        {
            ...
            while (best_b == long.MaxValue && max_Iterations-- > 0)
            {
                ...
                for (int b = 2; b <= max_b; b++)
                {
                    for (int h = 2; h <= max_h; h++)
                    {
                        ...
                        */
                        ...
                        bool alpha_one_OK = false;
                        bool alpha_two_OK = false;
                ...
                {
                    PrintToScreen.WriteLine("Warning: Computing b and k looks like a lot of work!");
            ...
            if (best_b == long.MaxValue)
            {
                ...
                // no way around exact quantile search.
                resultArr[0] = 1;
                resultArr[1] = long.MaxValue;
                resultArr[2] = long.MaxValue;
            }
            else
            {
                resultArr[0] = best_b;
