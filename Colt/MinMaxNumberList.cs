...
namespace HC.Analytics.Colt
{
    ...
    */
    ...
    ////import bitvector.QuickBitVector;
    /**
     * Resizable compressed list holding numbers; based on the fact that a value in a given interval need not take more than <tt>log(Max-Min+1)</tt> bits; implemented with a <tt>bitvector.BitVector</tt>.
     ...
     * If you need to store <tt>float</tt> or <tt>double</tt> values, you must set the minimum and maximum to <tt>[int.MinValue,int.MaxValue]</tt> or <tt>[long.MinValue,long.MaxValue]</tt>, respectively.
     * <p>
     * Although access methods are only defined on <tt>long</tt> values you can also store
     * all other primitive data types: <tt>bool</tt>, <tt>byte</tt>, <tt>short</tt>, <tt>int</tt>, <tt>long</tt>, <tt>float</tt>, <tt>double</tt> and <tt>char</tt>.
     ...
     * if (d1!=d2) PrintToScreen.WriteLine("This is impossible!");
     *
     ...
     * if (f1!=f2) PrintToScreen.WriteLine("This is impossible!");
     * </pre>
     *
     * @see LongArrayList
    ...
    public class MinMaxNumberList : AbstractLongList
    {
        ...
        {
            setUp(minimum, maximum, initialCapacity);
        }
        ...
        public new void Add(long element)
        {
            ...
            int i = size*m_bitsPerElement;
            QuickBitVector.putLongFromTo(bits, element - m_minValue, i, i + m_bitsPerElement - 1);
            size++;
        }
        ...
        public void addAllOfFromTo(long[] elements, int from, int to)
        {
            ...
            int bitsPerElem = m_bitsPerElement;
            int bitsPerElemMinusOne = bitsPerElem - 1;
            long min = m_minValue;
            long[] theBits = bits;

            // now let's go.
            ensureCapacity(size + to - from + 1);
            int firstBit = size*bitsPerElem;
            int i = from;
        ...
        public static int bitsPerElement(long minimum, long maximum)
        {
            ...
            {
                bits = (int) Math.Round(Math.Ceiling(
                                            Arithmetic.Log(2, 1 + maximum - minimum)));
            }
            else
            {
                ...
                // e.g. if minimum==LONG.MinValue, maximum==LONG.MaxValue
        ...
        public override void ensureCapacity(int minCapacity)
        {
            ...
            if (minCapacity > oldCapacity)
            {
                ...
                vector.setSize(newCapacity*m_bitsPerElement);
                bits = vector.elements();
        ...
        {
            int i = index*m_bitsPerElement;
            return m_minValue + QuickBitVector.getLongFromTo(bits, i, i + m_bitsPerElement - 1);
        }
        ...
        public void partFromTo(int from, int to, BitVector qualificants, int qualificantsFrom, long[] part, int partFrom)
        {
            ...
            //BitVector tmpBitVector = new BitVector(bits, size*bitsPerElem);
        ...
        public new void setSizeRaw(int newSize)
        {
        ...
        public void setUp(long minimum, long maximum, int initialCapacity)
        {
            ...
            bits = QuickBitVector.makeBitVector(initialCapacity, m_bitsPerElement);
            capacity = initialCapacity;
            size = 0;
        }
        ...
        {
            m_bitsPerElement = bitsPerElement(minimum, maximum);
            if (m_bitsPerElement != 64)
            {
                m_minValue = minimum;
                // overflow or underflow in calculating "1+maxValue-minValue"
                // happens if signed long representation is too short for doing unsigned calculations
                // e.g. if minValue==LONG.MinValue, maxValue=LONG.MaxValue
        ...
        {
            return new BitVector(bits, capacity*m_bitsPerElement);
        }
