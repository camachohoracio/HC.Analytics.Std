...
namespace HC.Analytics.Colt
{
    ...
    */
    ...
    ////import RngWrapper;
    /**
     * Space and time efficiently computes a sorted <i>Simple RngWrapper Sample Without Replacement (SRSWOR)</i>, that is, a sorted set of <tt>n</tt> random numbers from an interval of <tt>N</tt> numbers;
     ...
     *
     * <p><b>Sorted Simple RngWrapper Sample Without Replacement (SRSWOR):</b>
     ...
     *
     * <p><b>RngWrapper number generation:</b> By default uses <tt>MersenneTwister</tt>, a very strong random number generator, much better than <tt>RngWrapper</tt>.
     ...
     *
     * <p><b>Implementation:</b> after J.S. Vitter, An Efficient Algorithm for Sequential RngWrapper Sampling,
    ...
    [Serializable]
    public class RandomSampler : PersistentObject
    {
        //[Serializable]public class RandomSampler : Object : java.io.Serializable {
        private long my_low;
        private long my_n;
        private long my_N;
        public RngWrapper my_RandomGenerator;
        ...
        public RandomSampler(long n, long N, long low, RngWrapper randomGenerator)
        {
            if (n < 0)
            {
            ...
            if (n > N)
            {
                throw new ArgumentException("n must by <= N");
            }
            my_n = n;
            my_N = N;
            my_low = low;
            ...
            my_RandomGenerator = randomGenerator;
        }
        ...
        public new Object Clone()
        {
            ...
            copy.my_RandomGenerator = (RngWrapper) my_RandomGenerator.Clone();
            return copy;
        }
        ...
        public void nextBlock(int count, long[] values, int fromIndex)
        {
            if (count > my_n)
            {
                throw new ArgumentException("RngWrapper sample exhausted.");
            }
            if (count < 0)
            {
        ...
        public static void rejectMethodD(long n, long N, int count, long low, long[] values, int fromIndex,
                                  ...
                                  RngWrapper randomGenerator)
        {
            ...
            if (count > n)
            {
                throw new ArgumentException("count must not be greater than n");
            }
            if (randomGenerator == null)
            {
        ...
        public static void sampleMethodA(long n, long N, int count, long low, long[] values, int fromIndex,
                                         RngWrapper randomGenerator)
        {
        ...
        public static void sampleMethodD(long n, long N, int count, long low, long[] values, int fromIndex,
                                         RngWrapper randomGenerator)
        {
            double nreal, Nreal, ninv, nmin1inv, U, X, Vprime, y1, y2, top, bottom, negSreal, qu1real;
        ...
        public static void test(long n, long N, long low, int chunkSize, int times)
        {
            ...
            for (long t = times; --t >= 0;)
            {
                ...
                if (toDo > 0)
                {
                    ...
                    Log.print((values[toDo-1]));
                    Log.println("]");
                    */
                }
            }
            timer.stop();
            PrintToScreen.WriteLine("single run took " + timer.elapsedTime()/times);
        ...
        public static void testNegAlphaInv(string[] args)
        {
            /*
            long N = long.Parse(args[0]);
            int chunkSize = int.Parse(args[1]);
            ...
            for (int i=0; i<alphas.Length; i++) {
                ...
                Console.Write(" n="+N/80+" --> ");
                test(N/80,N,0,chunkSize);

                Console.Write(" n="+N/40+" --> ");
