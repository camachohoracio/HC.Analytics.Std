...
namespace HC.Analytics.Colt
{
    ...
    */
     ...
     * Also see {@link bin.QuantileBin1D}, demonstrating how this //package can be used.
     *
    ...
    [Serializable]
    public class QuantileFinderFactory : Object
    {
        /**
        ...
        public static long[] known_N_compute_B_and_K_quick(long N, double epsilon)
        {
            ...
            for (int b = 2; b <= maxBuffers; b++)
            {
                ...
                if (h > int.MinValue)
                {
                    ...
                    long tmpK = (long) (Math.Ceiling(N_double/value));
            ...
            else
            {
                ...
                b2 = 1; // the only possible solution without violating the 
                                                          ...
                                                          double[] returnSamplingRate)
        {
            ...
            for (long b = 2; b < maxBuffers; b++)
            {
                for (long h = 3; h < maxHeight; h++)
                {
                    ...
                    long tmp = (long) Math.Ceiling(N_double/binomial);
                    if ((b*tmp < memory) &&
                    ...
                    if (delta > 0.0)
                    {
                        ...
                        long k = (long) Math.Ceiling(w*x*x/v);
                        if (b*k < memory)
                        {
                            ret_k = k;
                            ret_b = b;
            ...
            RngWrapper generator)
        {
            ...
            else
            {
                ...
                //    return new KnownDoubleQuantileEstimator(
                //        (int)b2, 
                //        (int)k2, 
                //        N, 
                //        samplingRate[0], 
        ...
        public static long[] unknown_N_compute_B_and_K_raw(double epsilon, double delta, int quantiles)
        {
                    ...
                    for (int h = 2; h <= max_h; h++)
                    {
                        ...
                        */
                        ...
                        if (alpha_one_OK || alpha_two_OK)
                        {
                            ...
                            long k = (long) Math.Ceiling(Math.Max(d/alpha, (h + 1)/(2.0*epsilon)));
                            if (k > 0)
                            {
                                // valid solution?
                                long memory = b*k;
            ...
            if (best_b == long.MaxValue)
            {
                ...
                // no way around exact quantile search.
                result2[0] = 1;
                result2[1] = long.MaxValue;
                result2[2] = long.MaxValue;
            }
            else
            {
                result2[0] = best_b;
