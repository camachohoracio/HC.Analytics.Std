...
namespace HC.Analytics.Colt
{
    ...
    */
     ...
     */
    //[Serializable]public abstract class RandomEngine extends edu.cornell.lassp.houle.RngPack.RandomSeedable implements cern.colt.function.DoubleFunction, cern.colt.function.IntFunction {
    [Serializable]
    public abstract class RandomEngine : PersistentObject, DoubleFunction, IntFunction
    {
        ...
        */
        ...
        {
            return new MersenneTwister((int)
        ...
        public double raw()
        {
            ...
            do
            {
                ...
                intNextInt = NextInt(); // in [Integer.MIN_VALUE,Integer.MAX_VALUE]-interval
            }
            while (intNextInt == 0);

            // transform to (0.0,1.0)-interval
            // 2.3283064365386963E-10 == 1.0 / Math.pow(2,32)
            return (intNextInt & 0xFFFFFFFFL)*2.3283064365386963E-10;
