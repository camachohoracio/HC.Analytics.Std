...
namespace HC.Analytics.Colt
{
     ...
     * check out {@link Descriptive} in combination with methods {@link #elements()} and {@link #sortedElements()}. 
     * <p>
     * <b>Implementation</b>:
    ...
    {
        // Never ever use "size" as it would be intuitive!
        // This class abuses "size". "size" DOES NOT REFLECT the number of elements contained in the receiver!
        // Instead, "size" reflects the number of elements incremental stats computation has already processed.

        /**
         * The elements contained in this bin.
         */
        public DoubleArrayList m_elements;
        ...
        // cache states
        public bool m_isIncrementalStatValid = true;
        public bool m_isSorted = true;
        //public bool isSkewValid = true;
        //public bool isKurtosisValid = true;

        public bool m_isSumOfInversionsValid = true;
        public bool m_isSumOfLogarithmsValid = true;
        public DoubleArrayList m_sortedElements;
        //public bool isSumOfPowersValid = true;

        /**
         * Constructs and returns an empty bin; implicitly calls {@link #setFixedOrder(bool) setFixedOrder(false)}.
         */

        public DynamicBin1D()
        {
            Clear();
            m_elements = new DoubleArrayList();
            m_sortedElements = new DoubleArrayList(0);
            m_fixedOrder = false;
            m_hasSumOfLogarithms = true;
            m_hasSumOfInversions = true;
        }
        ...
        {
            m_elements.addAllOfFromTo(list, from, to);
            invalidateAll();
        }
        ...
        // Sum( x[i]*x[i] ) 
        bin.aggregate(Functions.plus,Functions.square);
        --> 14
        </pre>
        ...
        */
        ...
        public double aggregate(DoubleDoubleFunction aggr, DoubleFunction f)
        {
            ...
            double a = f.Apply(m_elements.getQuick(s - 1));
            for (int i = s - 1; --i >= 0;)
            {
                a = aggr.Apply(a, f.Apply(m_elements.getQuick(i)));
            }
            return a;
        }
        ...
        public new Object Clone()
        {
            ...
            if (m_elements != null)
            {
                Clone.m_elements = Clone.m_elements.Copy();
            }
            if (m_sortedElements != null)
            {
                Clone.m_sortedElements = Clone.m_sortedElements.Copy();
            }
            return Clone;
        }
        ...
        public double covariance(DynamicBin1D other)
        {
            lock (other)
            {
                ...
                for (int i = Size(); --i >= 0;)
                {
                    s += m_elements.getQuick(i)*other.m_elements.getQuick(i);
                }
         ...
         * Concerning the order in which elements are returned, see {@link #setFixedOrder(bool)}.
         * @return a copy of the currently stored elements.
         */

        public DoubleArrayList elements()
        {
            // safe since we are already .
         ...
         * ...
         * double sinSum = 0;
         *  (dynamicBin) { // lock out anybody else
         ...
         * 	   }
         * }
         * PrintToScreen.WriteLine(sinSum);
         * ...
         * </pre>
         *
         * Concerning the order in which elements are returned, see {@link #setFixedOrder(bool)}.
        ...
        public DoubleArrayList elements_unsafe()
        {
            return m_elements;
        }
        ...
        public new bool Equals(Object object_)
        {
            if (!(object_ is DynamicBin1D))
            {
                return false;
            }
            if (!base.Equals(object_))
            {
                return false;
            }

            DynamicBin1D other = (DynamicBin1D) object_;
        ...
        private static bool includes(double[] array1, double[] array2, int first1, int last1, int first2, int last2)
        {
        ...
        private AbstractDoubleIntMap frequencyMap()
        {
        ...
        /**
         * Returns <tt>int.MaxValue</tt>, the maximum order <tt>k</tt> for which sums of powers are retrievable.
         * @see #hasSumOfPowers(int)
         * @see #sumOfPowers(int)
         */

        public new int getMaxOrderForSumOfPowers()
        {
            return int.MaxValue;
        }

        /**
         * Returns <tt>int.MinValue</tt>, the minimum order <tt>k</tt> for which sums of powers are retrievable.
         * @see #hasSumOfPowers(int)
         * @see #sumOfPowers(int)
         */

        public new int getMinOrderForSumOfPowers()
        {
            return int.MinValue;
        }
        ...
        {
            m_isSorted = false;
            m_isIncrementalStatValid = false;

            //isSkewValid = false;
            //isKurtosisValid = false;

            m_isSumOfInversionsValid = false;
            m_isSumOfLogarithmsValid = false;
        }
        ...
        public new double moment(int k, double c)
        {
            ...
            return Descriptive.moment(m_elements, k, c);
        }
        ...
        public new DoubleArrayList quantiles(DoubleArrayList percentages)
        {
        ...
        public bool removeAllOf(DoubleArrayList list)
        {
            bool changed = m_elements.removeAll(list);
            if (changed)
            {
                clearAllMeasures();
                invalidateAll();
                ...
                if (m_fixedOrder)
                {
                    m_sortedElements.removeAll(list);
                    m_isSorted = true;
                }
            }
            return changed;
         ...
         * @throws ArgumentException if <tt>!withReplacement && n > Size()</tt>.
         * @see sampling
         */

        public void sample(int n, bool withReplacement, RngWrapper randomGenerator, DoubleBuffer buffer)
        {
            ...
            if (!withReplacement)
            {
                ...
                for (int i = n; --i >= 0;)
                {
                    ...
                    {
                        buffer.Add(m_elements.getQuick(i));
                    }
                }
            }
            else
            {
                // with
                RngWrapper uniform = new RngWrapper();
                int s = Size();
                for (int i = n; --i >= 0;)
                {
                    buffer.Add(m_elements.getQuick(uniform.NextInt(0, s - 1)));
                }
                buffer.flush();
            }
        }
        ...
        DynamicBin1D X = new DynamicBin1D();
        DynamicBin1D Y = new DynamicBin1D();
        X.addAllOf(new DoubleArrayList(v1));
        Y.addAllOf(new DoubleArrayList(v2));
        RngWrapper random = new MersenneTwister();
        ...
        Functions F = Functions.functions;
        PrintToScreen.WriteLine("p-value="+ (boot.aggregate(Functions.plus, Functions.greater(0)) / boot.Size()));
        PrintToScreen.WriteLine("left 90% confidence interval = ("+boot.quantile(0.9) + ",infinity)");
        ...
        @see GenericPermuting#permutation(long,int)
        */

        public DynamicBin1D sampleBootstrap(DynamicBin1D other, int resamples, RngWrapper randomGenerator,
                                            BinBinFunction1D function)
        {
            ...
            for (int i = resamples; --i >= 0;)
            {
                ...
                sample(s1, true, randomGenerator, buffer1);
        ...
        public void setFixedOrder(bool fixedOrder)
        {
            //if (Size() > 0) throw new HCException("must be called before starting to Add elements.");
            m_fixedOrder = fixedOrder;
        }
        ...
        public void Sort()
        {
            if (!m_isSorted)
            {
                ...
                {
                    m_sortedElements.Clear();
                    m_sortedElements.addAllOfFromTo(m_elements, 0, m_elements.Size() - 1);
         ...
         * Concerning the memory required for operations involving sorting, see {@link #setFixedOrder(bool)}.
         *
        ...
        public DoubleArrayList sortedElements_unsafe()
        {
            Sort();
            if (m_fixedOrder)
            {
        ...
        {
            Descriptive.standardize(m_elements, mean, standardDeviation);
            clearAllMeasures();
            invalidateAll();
            m_size = 0;
        }
        ...
        public new double sumOfPowers(int k)
        {
            // no chaching for this measure
            if (k >= -1 && k <= 2)
            {
                return base.sumOfPowers(k);
            }

            return Descriptive.sumOfPowers(m_elements, k);
        }
        ...
        public new string ToString()
        {
            ...
            IntArrayList frequencies_ = new IntArrayList();
            frequencies(distinctElements, frequencies_);
            if (distinctElements.Size() < 100)
            {
                // don't cause unintended floods
                buf.Append("Distinct elements: " + distinctElements + Environment.NewLine);
                buf.Append("Frequencies: " + frequencies_ + Environment.NewLine);
            }
            else
            {
        ...
        public void updateIncrementalStats()
        {
            ...
            arguments[1] = m_max;
            arguments[2] = m_sum;
            arguments[3] = m_sum_xx;

            Descriptive.incrementalUpdate(m_elements, m_size, m_elements.Size() - 1, arguments);

            // store the new parameters back
            m_min = arguments[0];
            m_max = arguments[1];
            m_sum = arguments[2];
            m_sum_xx = arguments[3];

            m_isIncrementalStatValid = true;
            m_size = m_elements.Size(); // next time we don't need to redo the stuff we have just done...
        }
        ...
        {
            m_sumOfInversions = Descriptive.sumOfInversions(m_elements, 0, Size() - 1);
            m_isSumOfInversionsValid = true;
        }
        ...
        {
            m_sumOfLogarithms = Descriptive.sumOfLogarithms(m_elements, 0, Size() - 1);
            m_isSumOfLogarithmsValid = true;
        }
