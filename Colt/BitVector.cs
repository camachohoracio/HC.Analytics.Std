...
namespace HC.Analytics.Colt
{
    ...
    */
     ...
     * Any attempt to access a bit at an <tt>index&lt;0 || index&gt;=Size()</tt> will throw an <tt>HCException</tt>.
     * <p>
     ...
     * Operations involving two bitvectors (like AND, OR, XOR, etc.) will throw an <tt>ArgumentException</tt> if the secondary bit vector has a size smaller than the receiver.
     * <p>
     ...
     * <p>
     * Why this class? Fist, <tt>bool[]</tt> take one byte per stored bit. This class takes one bit per stored bit.
     * Second, many applications find the semantics of <tt>BitSet</tt> not particularly helpful for their needs.
    ...
    [Serializable]
    public class BitVector : PersistentObject
    {
        /*
         ...
         * significant bit and 63 refers to the most significant bit).
         *
         * @throws ArgumentException if <tt>size &lt; 0 || size &gt; bits.Length*64</tt>.
         */
        ...
        public void and(BitVector other)
        {
            ...
            checkSize(other);
            long[] theBits = m_bits; // cached for speed.
            long[] otherBits = other.m_bits; //cached for speed.
        ...
        public void checkSize(BitVector other)
        {
            if (nbits > other.Size())
            {
                throw new ArgumentException("Incompatible sizes: size=" + nbits + ", other.Size()=" + other.Size());
            }
        }
         ...
         * @throws	HCException if <tt>bitIndex&lt;0 || bitIndex&gt;=Size()</tt>
         */
        ...
        public void clear(int bitIndex)
        {
            ...
            {
                throw new HCException((bitIndex.ToString()));
            }
            QuickBitVector.clear(m_bits, bitIndex);
        }
        ...
        {
            BitVector Clone = (BitVector) base.Clone();
            if (m_bits != null)
            {
         ...
         * <pre>((BitVector)obj).get(k) == get(k)</pre>
         * must be true.
         *
        ...
        public override bool Equals(Object obj)
        {
            ...
            int i_ = fullUnits*QuickBitVector.BITS_PER_UNIT;
            for (int times = numberOfBitsInPartialUnit(); --times >= 0;)
            {
                if (get(i_) != other.get(i_))
                {
                    return false;
                }
                i_++;
         ...
         * @throws HCException if (<tt>Size()&gt;0 && (from&lt;0 || from&gt;to || to&gt;=Size())</tt>).
         */

        public bool forEachIndexFromToInState(int from, int to, bool state, IntProcedure procedure)
        {
            /*
            ...
            checkRangeFromTo(from, to, nbits);
             long[] theBits = bits; // cached for speed
            ...
            */
            ...
            //PrintToScreen.WriteLine("from="+from+", to="+to+", bit="+state);
            ...
            if (bitIndex > 0)
            {
                ...
                //PrintToScreen.WriteLine("partialWidth1="+partialWidth);
            ...
            //PrintToScreen.WriteLine("partialWidth2="+partialWidth);
            ...
            //PrintToScreen.WriteLine("fromUnit="+fromUnit+", toUnit="+toUnit);
            ...
            //PrintToScreen.WriteLine("trail with i="+i);	
        ...
        public bool get(int bitIndex)
        {
            ...
            return QuickBitVector.get(m_bits, bitIndex);
        }
         ...
         * @throws	HCException if <tt>from&lt;0 || from&gt;=Size() || to&lt;0 || to&gt;=Size() || to-from+1<0 || to-from+1>64</tt>
         */
        ...
        public long getLongFromTo(int from, int to)
        {
            ...
            {
                throw new HCException("from:" + from + ", to:" + to);
            }
            return QuickBitVector.getLongFromTo(m_bits, from, to);
        }
        ...
        public bool getQuick(int bitIndex)
        {
            return QuickBitVector.get(m_bits, bitIndex);
        }
         ...
         * @exception HCException if (<tt>Size()&gt;0 && (from&lt;0 || from&gt;to || to&gt;=Size())</tt>).
         */

        public int indexOfFromTo(int from, int to, bool state)
        {
        ...
        public void put(int bitIndex, bool value)
        {
            if (bitIndex < 0 || bitIndex >= nbits)
            {
                throw new HCException((bitIndex.ToString()));
            ...
            if (value)
            {
                QuickBitVector.set(m_bits, bitIndex);
            }
            else
            {
                QuickBitVector.clear(m_bits, bitIndex);
        ...
        public void putLongFromTo(long value, int from, int to)
        {
            ...
            QuickBitVector.putLongFromTo(m_bits, value, from, to);
        }
        ...
        public void putQuick(int bitIndex, bool value)
        {
            if (value)
            {
                QuickBitVector.set(m_bits, bitIndex);
         ...
         * @throws HCException if <tt>Size()&gt;0 && (from&lt;0 || from&gt;to || to&gt;=Size() || sourceFrom<0 || sourceFrom+to-from+1>source.Size()))</tt>.
         */
        ...
        public void replaceFromToWith(int from, int to, BitVector source, int sourceFrom)
        {
            ...
            if (source.m_bits == m_bits && from <= sourceFrom && sourceFrom <= to)
            {
            ...
            long[] theBits = m_bits; // cached for speed.
            long[] sourceBits = source.m_bits; // cached for speed.
            ...
            */
            ...
            {
                long val2 = QuickBitVector.getLongFromTo(sourceBits, sourceFrom, sourceFrom + bitsPerUnitMinusOne);
                QuickBitVector.putLongFromTo(theBits, val2, from, from + bitsPerUnitMinusOne);
        ...
        public void replaceFromToWith(int from, int to, bool value)
        {
            if (nbits == 0 || to == from - 1)
            {
                return;
        ...
        #region Nested type: IndexProcedure

        private class IndexProcedure : IntProcedure
        {
            public int foundPos = -1;
