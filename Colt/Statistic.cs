...
namespace HC.Analytics.Colt
{
    ...
    */
    ...
    Also see {@link cern.jet.stat} and {@link bin}, in particular {@link DynamicBin1D}.
    <p>
    Examples:
    ...
    */
    ...
    {
        private static readonly Functions F = Functions.m_functions;
        /**
         ...
         */
        public static VectorVectorFunction BRAY_CURTIS = new VectorVectorFunction1_(F);

        /**
         * Canberra distance function; <tt>Sum( abs(x[i]-y[i]) / abs(x[i]+y[i]) )</tt>.
         */
        public static VectorVectorFunction CANBERRA = new VectorVectorFunctionCanberra(F);
        public static VectorVectorFunction EUCLID = new VectorVectorFunction_(F);
         ...
         */
        public static VectorVectorFunction MANHATTAN = new VectorVectorFunctionManhattan(F);
        public static VectorVectorFunction MAXIMUM = new VectorVectorFunction2_(F);
         ...
         * @see Formatter
         * @see BinFunction1D
         * @see bin.BinFunctions1D
         */

        public static DoubleMatrix2D aggregate(
        ...
        <br>
        Tip: Use <tt>PrintToScreen.WriteLine(bin(vector))</tt> to print most measures computed by the bin. Example:
        <table>
        <td class="PRE"> 
        <pre>
        Size: 20000
        ...
        */
        ...
        If you use Converter.ToString(histo) on the result, the OLAP cube of x-"column" vs. y-"column" , summing the weights "column" will be printed.
        ...
        @throws ArgumentException if <tt>x.Size() != y.Size() || y.Size() != weights.Size()</tt>.
        */
        ...
        If you use Converter.ToString(histo) on the result, the OLAP cube of x-"column" vs. y-"column" vs. z-"column", summing the weights "column" will be printed.
        ...
        Then fills the histogram.
        @return the histogram containing the cube.
        @throws ArgumentException if <tt>x.Size() != y.Size() || x.Size() != z.Size() || x.Size() != weights.Size()</tt>.
        */
        ...
        public static IHistogram3D cube(DoubleMatrix1D x, DoubleMatrix1D y, DoubleMatrix1D z, DoubleMatrix1D weights)
        {
            ...
            IHistogram3D histo = new Histogram3D("Cube", xaxis, yaxis, zaxis);
        ...
        public static void demo1()
        {
            ...
            PrintToScreen.WriteLine("\ncovar1=" + covariance(A));
            //PrintToScreen.WriteLine(correlation(covariance(A)));
            //PrintToScreen.WriteLine(distance(A,EUCLID));


            //PrintToScreen.WriteLine(matrixpattern.Converting.toHTML(A.ToString()));
            //PrintToScreen.WriteLine(matrixpattern.Converting.toHTML(covariance(A).ToString()));
            //PrintToScreen.WriteLine(matrixpattern.Converting.toHTML(correlation(covariance(A)).ToString()));
            //PrintToScreen.WriteLine(matrixpattern.Converting.toHTML(distance(A,EUCLID).ToString()));
        }
        ...
        public static void demo2(int rows, int columns, bool print)
        {
            PrintToScreen.WriteLine("\n\ninitializing...");
            DoubleFactory2D factory = DoubleFactory2D.dense;
            ...
            PrintToScreen.WriteLine("benchmarking correlation...");
            ...
            if (print)
            {
                PrintToScreen.WriteLine("printing result...");
                PrintToScreen.WriteLine(corr);
            }
        ...
        public static IHistogram3D histogram(IHistogram3D histo, DoubleMatrix1D x, DoubleMatrix1D y, DoubleMatrix1D z,
                                             DoubleMatrix1D weights)
        {
            if (x.Size() != y.Size() || x.Size() != z.Size() || x.Size() != weights.Size())
            {
        ...
        public static void main(string[] args)
        {
            ...
            bool print = args[2].Equals("print");
        ...
        @throws ArgumentException if <tt>! (0 <= rowFraction <= 1 && 0 <= columnFraction <= 1)</tt>.
        @see RandomSampler
        */

        public static DoubleMatrix1D viewSample(DoubleMatrix1D matrix, double fraction, RngWrapper randomGenerator)
        {
            ...
            {
                randomGenerator = new RngWrapper((int) (DateTime.MinValue - DateTime.Now).TotalMilliseconds);
            }
        ...
        */
        ...
        @param randomGenerator a uniform random number generator; set this parameter to <tt>null</tt> to use a default generator seeded with the current time.
        @return the sampling view.
        @throws ArgumentException if <tt>! (0 <= sliceFraction <= 1 && 0 <= rowFraction <= 1 && 0 <= columnFraction <= 1)</tt>.
        @see RandomSampler
        */

        public static DoubleMatrix3D viewSample(DoubleMatrix3D matrix, double sliceFraction, double rowFraction,
                                                double columnFraction, RngWrapper randomGenerator)
        {
            // check preconditions and allow for a little tolerance
            double epsilon = 1e-09;
        ...
        private static DoubleMatrix2D xdistanceOld(DoubleMatrix2D matrix, int norm)
        {
            ...
            if (norm==EUCLID) function = Functions.chain(Functions.square,Functions.minus);
            else if (norm==BRAY_CURTIS) function = Functions.chain(Functions.abs,Functions.minus);
            else if (norm==CANBERRA) function = new DoubleDoubleFunction() {
                public  double Apply(double a, double b) {	return Math.Abs(a-b) / Math.Abs(a+b);}
            };
            else if (norm==MAXIMUM) function = Functions.chain(Functions.abs,Functions.minus);
            else if (norm==MANHATTAN) function = Functions.chain(Functions.abs,Functions.minus);
            else throw new ArgumentException("Unknown norm");

            // work out all permutations
            for (int i=columns; --i >= 0; ) {
                for (int j=i; --j >= 0; ) {
                    double d = 0;
                    if (norm==EUCLID) d = Math.Sqrt(cols[i].aggregate(cols[j], Functions.plus, function));
                    else if (norm==BRAY_CURTIS) d = cols[i].aggregate(cols[j], Functions.plus, function) / cols[i].aggregate(cols[j], Functions.plus, Functions.plus);
                    else if (norm==CANBERRA) d = cols[i].aggregate(cols[j], Functions.plus, function);
                    else if (norm==MAXIMUM) d = cols[i].aggregate(cols[j], Functions.Max, function);
                    else if (norm==MANHATTAN) d = cols[i].aggregate(cols[j], Functions.plus, function);
                    distance.setQuick(i,j,d);
                    distance.setQuick(j,i,d); // symmetric
                }
            }
