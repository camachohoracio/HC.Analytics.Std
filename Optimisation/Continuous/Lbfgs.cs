 ...
 */
//package taglink.com.dataStructures.mixtureGaussian.riso.numerical;
 ...
 * 	,       large scale optimisation methods'' Mathematical Programming 45
 * 	,       (1989), pp. 503-528.
...
namespace HC.Analytics.Optimisation.Continuous
{
    public class Lbfgs
    {
        ...
        private static readonly int[] info = new int[1];
        private static readonly int[] nfev = new int[1];
        private static readonly double[] stp = new double[1];
        ...
        private static double ftol;
        private static double gnorm;
        public static double gtol = 0.9;

        private static int i,
                           inmc;

        private static int iscn;

        private static int ispt;
        private static int iter;
        private static int iycn;

        private static int iypt,
                           maxfev;

        private static int nfun;
        private static int npt;
        private static int point;
        public static double[] solution_cache;
        private static double sq;
        private static double stp1;
        public static double stpmax = 1e20;
        public static double stpmin = 1e-20;

        private static double[] w;
        private static double xnorm;
         ...
         *     Min f(x),    x = (x1,x2,...,x_n),
         * </pre>
         * //usingthe limited-memory BFGS method. The routine is especially
         ...
         * a diagonal matrix <code>Hk0</code>, //usinginformation from the previous M steps.
         ...
         * for large scale optimisation", by D. Liu and J. Nocedal,
         ...
         *		</ul>
         *
         *	@throws Lbfgs.Exception
         */

        public static void lbfgs(int n, int m, double[] x, double f, double[] g,
                                 bool diagco, double[] diag, int[] iprint,
                                 double eps, double xtol, int[] iflag)
        {
            var execute_entire_while_loop = false;
            ...
            if (iflag[0] == 0)
            {
                ...
                Array.Copy(x, 0, solution_cache, 0, n);
                ...
                {
                    iflag[0] = -3;
                    throw new HCException("Improper input parameters  (n or m are not positive.)");
                }
            ...
            while (true)
            {
                ...
                Mcsrch.mcsrch(n, x, f, g, w, ispt + point*n, stp, ftol, xtol, maxfev,
                              info, nfev, diag);
        ...
        public static void lb1(int[] iprint, int iter, int nfun, double gnorm, int n,
                               int m, double[] x, double f, double[] g, double[] stp,
                               bool finish)
        {
            int i;
            ...
            {
                PrintToScreen.WriteLine("*************************************************");
                PrintToScreen.WriteLine("  n = " + n + "   number of corrections = " + m +
                                        "\n       initial values");
                PrintToScreen.WriteLine(" f =  " + f + "   gnorm =  " + gnorm);
                if (iprint[2 - 1] >= 1)
                {
                    PrintToScreen.Write(" vector x =");
                    for (i = 1; i <= n; i++)
                    {
                        PrintToScreen.Write("  " + x[i - 1]);
                    }
                    PrintToScreen.WriteLine("");

                    PrintToScreen.Write(" gradient vector g =");
                    for (i = 1; i <= n; i++)
                    {
                        PrintToScreen.Write("  " + g[i - 1]);
                    }
                    PrintToScreen.WriteLine("");
                }
                PrintToScreen.WriteLine("*************************************************");
                PrintToScreen.WriteLine("\ti\tnfn\tfunc\tgnorm\tsteplength");
            }
            else
            {
                ...
                if (iprint[1 - 1] != 0)
                {
                    if ((iter - 1)%iprint[1 - 1] == 0 || finish)
                    {
                        ...
                        PrintToScreen.WriteLine("\t" + iter + "\t" + nfun + "\t" + f + "\t" +
                                                gnorm + "\t" + stp[0]);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    PrintToScreen.WriteLine("\t" + iter + "\t" + nfun + "\t" + f + "\t" + gnorm +
                                            "\t" + stp[0]);
                }
        ...
        public static double ddot(int n, double[] dx, int ix0, int incx, double[] dy,
                                  int iy0, int incy)
        {
            double dtemp;
            int i, ix, iy, m, mp1;
            ...
            for (i = mp1; i <= n; i += 5)
            {
                dtemp = dtemp + dx[ix0 + i - 1]*dy[iy0 + i - 1] + dx[ix0 + i + 1 -
                                                                     1]*dy[iy0 + i + 1 - 1] +
                        dx[ix0 + i + 2 - 1]*dy[iy0 + i + 2 - 1] +
                        dx[ix0 + i + 3 - 1]*dy[iy0 + i + 3 - 1] + dx[ix0 + i + 4 -
                                                                     1]*dy[iy0 + i + 4 - 1];
            }
