...
namespace HC.Analytics.Optimisation.Continuous
{
    ...
    {
        private static readonly bool[] brackt = new bool[1];
        private static readonly double[] dgx = new double[1];

        private static readonly double[] dgxm = new double[1];

        private static readonly double[] dgy = new double[1];

        private static readonly double[] dgym = new double[1];

        private static readonly double[] fx = new double[1];

        private static readonly double[] fxm = new double[1];

        private static readonly double[] fy = new double[1];

        private static readonly double[] fym = new double[1];
        private static readonly int[] infoc = new int[1];

        private static readonly double[] stx = new double[1];

        private static readonly double[] sty = new double[1];
        private static double dg;
        private static double dginit;
        private static double dgm;
        private static double dgtest;
        private static double finit;
        private static double fm;
        private static double ftest1;
        private static int j;
        private static double p5, p66;
        private static bool stage1;

        private static double stmax;

        private static double stmin;
         ...
         *		step length. On exit <code>stp</code> contains the estimate.
         *
        ...
        public static void mcsrch(int n, double[] x, double f, double[] g, double[] s,
                                  int is0, double[] stp, double ftol, double xtol,
                                  int maxfev, int[] info, int[] nfev, double[] wa)
        {
            ...
            if (info[0] != -1)
            {
                ...
                if (n <= 0 || stp[0] <= 0 || ftol < 0 || Lbfgs.gtol < 0 || xtol < 0 ||
                    Lbfgs.stpmin < 0 || Lbfgs.stpmax < Lbfgs.stpmin || maxfev <= 0)
                {
                    return;
                }
                ...
                if (dginit >= 0)
                {
                    PrintToScreen.WriteLine("The search direction is not a descent direction.");
                    return;
                }
                                  ...
                                  int[] info)
        {
            ...
            if ((brackt[0] &&
                 (stp[0] <= Math.Min(stx[0], sty[0]) ||
                  stp[0] >= Math.Max(stx[0], sty[0]))) ||
