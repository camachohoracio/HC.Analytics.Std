...
//using HC.Analytics.Probability.RngWrapper;

namespace HC.Analytics.Probability.Distributions.Discrete
{
    public class EmpiricalWalkerDist : AbstractUnivDiscrDist
    {
        protected int[] m_A;

        protected double[] m_cdf; // cumulative distribution function
         ...
         * there are 2*K*sizeof(double)  bytes of memory that are
         ...
         * Abramowitz and Stegun, p 950, citing: Marsaglia, RngWrapper variables
        ...
        {
            EmpiricalWalkerDist copy = (EmpiricalWalkerDist) Clone();
            if (m_cdf != null)
            {
                copy.m_cdf = (double[]) m_cdf.Clone();
            }
            if (m_A != null)
            {
                copy.m_A = (int[]) m_A.Clone();
            }
            if (m_F != null)
            {
                copy.m_F = (double[]) m_F.Clone();
            }
            return copy;
        }
        ...
        {
            if (k < 0 || k >= m_cdf.Length - 1)
            {
                return 0.0;
            }
            return m_cdf[k - 1] - m_cdf[k];
        }
       ...
       */
        ...
        {
            return ToString() + "(" + ((m_cdf != null) ? m_cdf.Length : 0) + ")";
        }
    }
}
